...
function updateTagSelect(selectEl,tagEl){const destination=state.destinations?.find((item)=>item.id===selectEl.value);const isForum=destination?.type==='forum';tagEl.innerHTML=`<option value="">${destination?.requireTag?'Choose required tag…':'No tag'}</option>`+(destination?.tags||[]).map((tag)=>`<option value="${tag.id}">${escapeHtml(tag.name)}</option>`).join('');tagEl.disabled=!isForum;if(!isForum)tagEl.value='';}
...
els.saveBtn.addEventListener('click',()=>saveCurrent());els.publishBtn.addEventListener('click',publishCurrent);els.cloneBtn.addEventListener('click',cloneCurrent);els.exportBtn.addEventListener('click',exportCurrent);els.deleteBtn.addEventListener('click',deleteCurrent);els.refreshBtn.addEventListener('click',()=>refreshBootstrap());els.newDraftBtn.addEventListener('click',openNewDraftDialog);els.undoBtn.addEventListener('click',undo);els.redoBtn.addEventListener('click',redo);els.destinationBtn.addEventListener('click',openDestinationDialog);els.historyBtn.addEventListener('click',openHistory);els.importDiscohookBtn.addEventListener('click',openDiscohookDialog);
els.newForum.addEventListener('change',()=>updateTagSelect(els.newForum,els.newTag));els.destinationSelect.addEventListener('change',()=>updateTagSelect(els.destinationSelect,els.destinationTag));els.discohookDestination.addEventListener('change',()=>updateTagSelect(els.discohookDestination,els.discohookTag));els.newDraftForm.addEventListener('submit',createDraft);els.destinationForm.addEventListener('submit',applyDestination);els.discohookForm.addEventListener('submit',importDiscohook);els.profileForm.addEventListener('submit',saveProfile);els.clearProfileBtn.addEventListener('click',clearProfile);
