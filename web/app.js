...
function renderEditorMeta() {
  ...
  const modified = state.scope.kind === 'p' && state.entity.publishedBuilder && (Boolean(state.entity.pendingDestination) || state.entity.title !== state.entity.publishedTitle || JSON.stringify(state.entity.builder) !== JSON.stringify(state.entity.publishedBuilder));
  ...
  els.publishBtn.textContent = state.scope.kind === 'd' ? 'Publish' : 'Republish';
...
function updateTagSelect(selectEl,tagEl,selected=[]){const destination=state.destinations?.find((item)=>item.id===selectEl.value);const isForum=destination?.type==='forum';const selectedIds=new Set(selected);const tagLabel=tagEl.closest('label');tagEl.innerHTML=(destination?.tags||[]).map((tag)=>`<option value="${tag.id}" ${selectedIds.has(tag.id)?'selected':''}>${escapeHtml(tag.name)}</option>`).join('');tagEl.disabled=!isForum;tagLabel?.classList.toggle('hidden',!isForum);tagLabel?.setAttribute('aria-hidden',String(!isForum));if(tagEl===els.newTag)els.newDraftForm.classList.toggle('new-draft-no-tags',!isForum);if(!isForum)[...tagEl.options].forEach((option)=>{option.selected=false;});}
...
async function openDestinationDialog(){if(!state.entity)return;try{await ensureDestinations();els.destinationSelect.innerHTML=state.destinations.map(destinationOptionHtml).join('');const pending=state.entity.pendingDestination||null;const current=pending?.destinationChannelId||state.entity.destinationChannelId||state.entity.forumChannelId||state.entity.forumId;if(current&&state.destinations.some((item)=>item.id===current))els.destinationSelect.value=current;updateTagSelect(els.destinationSelect,els.destinationTag,pending?.appliedTagIds||state.entity.appliedTagIds||[]);els.destinationHint.textContent=state.scope.kind==='p'?'This only stages the new destination. The existing Discord post remains unchanged until you confirm Republish.':'Draft destination changes without publishing.';els.destinationDialog.showModal();}catch(error){toast(error.message,'error');}}
async function applyDestination(event){event.preventDefault();const destination=state.destinations?.find((item)=>item.id===els.destinationSelect.value);const tagIds=validateSelectedTags(destination,els.destinationTag);if(!tagIds)return;try{if(state.dirty&&!await saveCurrent({silent:true}))return;const data=await api(`/api/entities/${state.scope.kind}/${encodeURIComponent(state.scope.id)}/destination`,{method:'PUT',body:{destinationId:els.destinationSelect.value,tagIds}});els.destinationDialog.close();state.scope=data.scope||state.scope;state.entity=data.entity;state.dirty=false;toast(state.scope.kind==='p'?'Destination staged. Use Republish to apply it.':'Destination updated.','success');await refreshBootstrap({keepSelection:false});await loadEntity(state.scope.kind,state.scope.id,{skipDirtyCheck:true});}catch(error){toast(error.message,'error');}}
...
try{await refreshBootstrap({keepSelection:false});const params=new URLSearchParams(location.search);const requestedKind=params.get('kind');const requestedId=params.get('id');const requested=[...(state.bootstrap?.drafts||[]),...(state.bootstrap?.posts||[])].find((item)=>item.kind===requestedKind&&item.id===requestedId);if(requested)await loadEntity(requested.kind,requested.id,{skipDirtyCheck:true});else if(requestedKind||requestedId)toast('The requested Timewizzard post could not be found.','error');renderUndoRedo();}catch(error){toast(error.message,'error');}
